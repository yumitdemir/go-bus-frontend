...
function Vehicles(props) {
    ...
    const getVehiclesQuery = useQuery({
        queryKey: ["posts",searchParams.get("page") ,searchParams.get("pageSize"),searchParams.get("filterQuery")],
        queryFn: () => {
    ...
    return (
        <div className={"w-full flex flex-col gap-4"}>
            ...
            {getVehiclesQuery.isLoading ? <span className=" self-center loading loading-spinner loading-lg"></span> :
                <TableMapper headers={headers} rows={tableData} containerClassName={"mx-auto"} showActions={true}/>}
            {getVehiclesQuery.isError ? <p className={"text-center text-error"}>Couldn't load data </p> : undefined}
            {!getVehiclesQuery.isLoading && !getVehiclesQuery.isError ?
